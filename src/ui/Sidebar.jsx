...
const Sidebar = ({ children }) => {
  const { user } = useUser();
  return (
    <aside className="md:relative h-full flex flex-col justify-evenly lg:justify-between md:row-start-1 md:row-span-2 p-4 bg-secondary-0">
      <ul className="flex flex-col gap-y-2 pt-10">{children}</ul>
      <div className="w-5/6 flex mt-20 lg:mt-0 lg:absolute lg:bottom-7 gap-x-5">
